	...
	public WebDriver initializeDriver() throws IOException
	{
		...
		if (browserName.contains("chrome")) {
			...
			if(browserName.contains("headless")) {
				options.addArguments("headless"); // Run chrome in headless mode ==> without visible chrome browser
			}
			WebDriverManager.chromedriver().setup(); 
			driver = new ChromeDriver(options);
			driver.manage().window().setSize(new Dimension(1440, 900)); // recommanded for headless mode to run in full screen
		}
